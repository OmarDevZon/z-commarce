...
const DashboardItem = ({
  ...
  const { name, link, icon, subMenu } = item;
  // const { subMenu, name, icon, link } = item;

  return (
    <>
      <div className={`${"mb-2"} text-sm rounded select-none group relative`}>
        {link ? (
          <div
            onClick={() => {
              setSubItemShow(name);
        ...
        {subMenu && subItemShow === name && toggle[name] && (
          <div
            className={`bg-blue-200 m-3 text-gray-700 py-2  ${
              openDrawer
                ? "absolute left-8 top-0 md:relative md:left-0"
                : "drop-shadow-xl relative left-0 md:absolute top-0 md:left-8 "
            }`}
          >
            {subMenu.map((menu: TSubMenu, i: number) => (
              <NavLink
                key={i}
                to={menu.subLink}
                className={({ isActive }) =>
              ...
              >
                ...
                <p style={{}} className={`whitespace-pre -ml-1`}>
                  {menu.subName}
                </p>
              </NavLink>
            ))}
